...
class Gemini_Server:
    ...
    def make_name(self):
        prompt = PromptTemplate(
            template="Make me a name for this conversation. here is the conversation : {conversation}",
            input_variables=["conversation"]
        )
        chain = prompt | self.model | StrOutputParser()

        result = chain.invoke({"conversation": self.history})
        return result
